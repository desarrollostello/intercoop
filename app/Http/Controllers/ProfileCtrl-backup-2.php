...
class ProfileCtrl extends Controller
{
    ...
    public function messages(Request $request)
    {
        ...
        //dd($first_messages);
        return view('profile.messages', ['tap_title'=>'Messages','messages'=>$msg ? $msg : null,'first_messages'=>$first_messages]);
    }

    public function messages_user($u_id)
    {
        try {
            $contact_id = decrypt($u_id);
        }catch (\Exception $e){
            Flash::toastWarning($e->getMessage());
            self::messages();
        }

        if(!$u_id){
            Flash::toastWarning("Field missing in request");
            self::messages();
        }
        if(!Checks::is_contact($contact_id)){
            $contact = User::find($contact_id)->first();
            Flash::toastWarning($contact->user_name." not is your contact");
            self::messages();
        }

        $user_id = Auth::user()->id;
        $msg = null;
        $messages = self::__listMessagesContacts($user_id);
        foreach ($messages as $key => $sub_msg){
            $msg[$key] = Message::select(DB::raw('*, IF(user_from = '.$user_id.', user_to, user_from) AS last_user'))
                ->where('id',$sub_msg->new_id)
                ->orderBy('created_at','desc')->get();
    ...
    public function contacts()
    {
        ...
        /*$contacts = User::whereExists(function($query){
            ...
            ->get();*/
        $contacts = Contacts::whereIn('contacts.status',[0,1])
    ...
    {
    	return view('profile.visitors', ['tap_title'=>'Visitors','visitors'=>null]);
    }
