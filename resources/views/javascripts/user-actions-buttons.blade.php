...
<script>
    $(function(){
        ...
        $('.delete-contact-button').click(function(){
            var element = $(this);
            mbox.confirm('Are you sure you want to delete <span style="text-decoration: underline;">{{ $user->user_name }}</span>?', function(yes) {
                if (yes) {
        ...
        @if(Checks::is_contact($user->id))
            $('.button-to-submit-message').click(function(){
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    data: {_token:$('#application-token').data('token'),_user:'{{ encrypt($user->id) }}',_message:input_message.val()},
                    beforeSend: function(){
