...
$(function(){
    ...
    $(document).on('click','.button-delete-profile-photo',function(e){
        var element = $(this);
        var data = {
    ...
    $(document).on('click', '.button-edit-profile-button', function(e){
        var modal = $('#main-modal');
        var element = $(this);
        $.ajax({
            url: base_url + 'getajax/editphoto',
            data: {photo:element.data('photo')},
            beforeSend: function(){
                $('.spinner-icon-hidden').removeClass('hidden-block');
                element.attr('disabled','').addClass('disabled');
            },
            success: function(data){
                $('.spinner-icon-hidden').addClass('hidden-block');
    ...
    $(document).on('change','#define-avatar-box',function(){
        var element = $(this);
        if(element.is(':checked')){
            $('#public-privacy').click();
        }
    });
    $(document).on('change','#justme-privacy',function(){
        var element = $(this);
        if(element.is(':checked')){
            $('#define-avatar-box').removeAttr('checked');
        }
    });
    $(document).on('submit','.form-edit-photo', function(e){
        var modal = $('#main-modal');
        var element = $(this);
        var submit_button = element.find('button[type="submit"]');
        var is_avatar = element.find('input[name="is_avatar"]');
        console.log(is_avatar);
        $.ajax({
            url: element.attr('action'),
            type: 'POST',
            data: element.serialize(),
            beforeSend: function(){
                $('.spinner-icon-hidden').removeClass('hidden-block');
                submit_button.attr('disabled','').addClass('disabled');
            },
            success: function(data){
                $('.spinner-icon-hidden').addClass('hidden-block');
                ...
                if(typeof data == 'object'){
                    if(data.status=='success') {
                        modal.closeModal();
                        if (is_avatar.is(':checked')) {
                            $('.circle.profile-avatar-small').attr('src', data.small).addClass('animated bounceIn');
                            $('.circle.profile-avatar-large').attr('src', data.large).addClass('animated bounceIn');
                        }
                        var $toastContent = $('<div class="green lighten-1">'+data.message+'</div>');
                    }else{
                        var $toastContent = $('<div class="red lighten-1">'+data.message+'</div>');
                    ...
                    var $toastContent = $('<div class="red lighten-1">Unknown error.</div>');
                }
                Materialize.toast($toastContent, 4000);
            }
        });
    ...
    $(window).scroll(function (e) {
        if($('#timeline-content').length > 0) {
            ...
            var last = content_articles.find('article').last().data('article');
            if(!last){
                return false;
            }
            if (dTop >= wHeight) {
    ...
    $(document).on('click','.morefeedcomments',function(e){
        var element = $(this);
