<script>
    ...
    $(document).on('submit','.comment-state-form',function (e) {
        var element = $(this);
        $.ajax({
            url: '<?php echo e(route("commentstate")); ?>',
            type: 'POST',
    ...
    $(document).on('click','.like-state-button',function(e){
        ...
        $.ajax({
            url: '<?php echo e(route("likestate")); ?>',
            type: 'POST',
                            ...
                            '<div class="card-content">'+
                                '<div class="row valign-wrapper no-margin">'+
                                    '<div class="col s2 no-padding" style="height: 3rem">'+
                                        '<a href="<?php echo e(route('accound', Auth::user()->id)); ?>">'+
                                            '<img src="<?php echo e(asset(Auth::user()->profile->avatar->small)); ?>" alt="avatar" width="40" class="circle profile-image" />&nbsp;'+
                                        '</a>'+
                                    '</div>'+
                                    '<div class="col s10 no-padding mpadding-l">'+
                                        '<a href="<?php echo e(route('accound', encrypt(Auth::user()->id))); ?>">'+
                                            '<span class="truncate blue-text text-lighten-1"><?php echo e(Auth::user()->user_name); ?></span>'+
                                        '</a>'+
    ...
    $(document).on('click','.like-photo-button',function (e) {
        ...
        var num_likes = parseInt(count_likes.text());
        $.ajax({
            url: '<?php echo e(route("likephoto")); ?>',
            type: 'POST',
